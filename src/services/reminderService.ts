...
export class ReminderService {
  ...
  async sendRemindersForPsychologist(psychologistId: string): Promise<number> {
    ...
    const sessions = await this.repository.findSessionsNeedingReminder(1, [psychologistId]);
    this.log.info(`[reminder-manual] ${sessions.length} sessão(ões) pendente(s) para psicólogo ${psychologistId}`);
    ...
    for (const session of sessions) {
      try {
        ...
        const psyName = await this.repository.findPsychologistNameById(psychologistId) ?? 'seu psicólogo';
        ...
        this.log.error({ err, sessionId: session.id }, '[reminder-manual] Falha ao enviar lembrete');
      }
    }
